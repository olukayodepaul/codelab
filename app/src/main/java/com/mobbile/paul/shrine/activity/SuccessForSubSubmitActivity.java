...
public class SuccessForSubSubmitActivity extends AppCompatActivity {
...
//        description.setText(getString(R.string.success_no_payment_string));
        if (!isPaymentRequired && isDollarPaymentRequired) {
            description.setText(String.format(Locale.getDefault(), "%s Amount in: $%d", getString(R.string.finalize_payments), amount/360));
            findViewById(R.id.paymentButton).setVisibility(View.GONE);
        } else if (!isDollarPaymentRequired && isPaymentRequired) {
            description.setText(String.format(Locale.getDefault(), "%s Amount in: NGN%d", getString(R.string.finalize_payments), amount));
            //findViewById(R.id.paypalPaymentButton).setVisibility(View.GONE);
        } else {
            description.setText(String.format(Locale.getDefault(), "%s Amount in $%d, and  NGN%d", getString(R.string.finalize_payments), amount/360, amount));
        }
        ...
        findViewById(R.id.paymentButton).setOnClickListener(v -> {
            Intent intent = new Intent(SuccessForSubSubmitActivity.this, PaymentActivity.class);
        ...
        findViewById(R.id.paypalPaymentButton).setOnClickListener(v -> {
            Intent intent = new Intent(SuccessForSubSubmitActivity.this, PayPal.class);
            intent.putExtra(ARGS_ORDER_ID, orderId);
            intent.putExtra(ARGS_ORDER_AMOUNT, amount);
            startActivity(intent);
            finish();
        ...
        completeButton.setOnClickListener(v -> {
            Intent intent = new Intent(SuccessForSubSubmitActivity.this, MainActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
            startActivity(intent);
            finish();
        });
    ...
    public void onBraintreeSubmit(View v) {
        DropInRequest dropInRequest = new DropInRequest().clientToken(clientToken)
                .disableCard();
        startActivityForResult(dropInRequest.getIntent(SuccessForSubSubmitActivity.this), REQUEST_CODE);
    }
